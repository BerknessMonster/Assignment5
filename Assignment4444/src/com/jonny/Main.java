...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		fileReadAndWrite.fileReader(users, user);
		//fileReadAndWrite.fileWriter(); make sure users is populated before line 21
		...
		while (loggedInUser == null && loginAttempts < 5) {
			...
			loggedInUser = userService.getValidUser(username, password, users, loggedInUser);
		...
		if (loggedInUser != null) {
			...
			while (loggedInUser != null) {
				...
				if (option == 0 && "super_user".equals(loggedInUser.getRole())) {
					...
					User userToUpdate = userService.getUserByUsername(usernameToUpdate, users);
